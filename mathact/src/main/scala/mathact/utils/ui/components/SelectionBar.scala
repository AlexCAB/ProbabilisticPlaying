...
extends FlowPanel(Left)() with UIComponent{
  ...
  //Abstract methods
  def selected(item:String, index:Int):Unit
  //Methods
  def setEnable(isEnable:Boolean):Unit = listCmp.comboBox.setEnabled(isEnable)
  def setList(list:List[String]):Unit = listCmp.setList(list)
  def setItem(index:Int):Unit = listCmp.setItem(index)}

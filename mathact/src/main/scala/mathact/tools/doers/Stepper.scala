...
extends Tool{
  ...
  /** Stop stepper and back to first move */
  def reset():Unit = {
    isReset = true
    doStop()
    nextStepIndex = 0
    dropList.setItem(nextStepIndex)}
  /** Set step by name */
  def setStep(stepName:String):Unit = steps.find(_.name == stepName) match {
    case Some(step) ⇒
      nextStepIndex = steps.indexOf(step)
      dropList.setItem(nextStepIndex)
    case _ ⇒
      throw new Exception("[Stepper.setStep] No step with name: " + stepName)}
  /** Disable UI */
  def disable():Unit = {
    dropList.setEnable(false)
    slider.setEnable(false)
    execBtn.setEnable(false)}
  /** Enable UI */
  def enable():Unit = {
    dropList.setEnable(true)
    slider.setEnable(true)
    execBtn.setEnable(true)}
  //Helpers
